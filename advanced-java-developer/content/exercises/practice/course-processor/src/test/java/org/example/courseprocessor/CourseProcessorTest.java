...
import static org.example.courseprocessor.CourseProcessor.*;
...
public class CourseProcessorTest {
    ...
    public void testGetJavaCourses() {
        List<Course> javaCourses = getJavaCourses(courses);
        assertEquals(4, javaCourses.size());
    }

    @Test
    public void testGetTrainerAvailability() {
        List<StartDateDurationAndTrainerName> availability = getTrainerAvailability(courses);
        assertEquals(of(2021,2,1), availability.getFirst().getStartDate());
        assertEquals(5, availability.getFirst().getNumDays());
        assertEquals("Alan Lavender", availability.getFirst().getTrainerName());
    }

    @Test
    public void testGetAvailabilityForGivenTrainer() {
        List<StartDateDurationAndTrainerName> availability = getTrainerAvailability(courses, "Stuart Bailey");
        assertEquals(of(2021,3,23), availability.getFirst().getStartDate());
        assertEquals(3, availability.size());
        assertEquals(4, availability.getFirst().getNumDays());
        assertEquals("Stuart Bailey", availability.getFirst().getTrainerName());
    }

    @Test
    public void testDistinctCourseNames() {
        List<String> courseNames = getDistinctCourseNames(courses);
        assertEquals(7, courseNames.size());
        assertEquals("HTML", courseNames.getFirst());
        assertEquals("Spring", courseNames.get(6));
    }

    @Test
    public void testGetDelegatesForCourse() {
        List<Person> delegates = getDelegatesForCourse(courses, "Java 1");
        assertEquals(20, delegates.size());
    }

    @Test
    public void testGetCoursesForDelegate() {
        List<Course> kelvinsCourses = getCoursesForDelegate(courses, "Kelvin Begley");
        assertEquals(1, kelvinsCourses.size());
        assertEquals("React", kelvinsCourses.getFirst().getName());
    }

    @Test
    public void testGetAverageNumberOfDelegatesForCourse() {
        OptionalDouble avg = getAverageNumberOfDelegatesForCourse(courses, "Python 1");
        assertEquals(10, avg.isPresent() ? avg.getAsDouble() : 0);
    }
}
